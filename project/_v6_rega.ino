...
#include <DHT.h>
#include <AESLib.h> // Biblioteca para criptografia AES
#include <base64.h> // Para codificar em Base64
...
// Chave AES (primeiros 16 bytes da chave decodificada de Base64 para AES-128)
const byte aes_key[] = {
...
const char* topic_estado = "IPB/IoT/PW2/TurnoD/Group2/EstadoSolo";
...
String encryptAES(String data) {
  // Padronizar o input para múltiplos de 16 bytes
  int input_len = data.length() + 1; // +1 para '\0'
  int padded_len = ((input_len + 15) / 16) * 16;
  uint8_t padded_input[padded_len] = {0};
  uint8_t output[padded_len];
  byte iv[16];
  memcpy(iv, aes_iv, 16); // Copiar IV fixo

  // Copiar dados para o input
  strncpy((char*)padded_input, data.c_str(), input_len);

  // Criptografar
  aesLib.encrypt(padded_input, padded_len, output, aes_key, 128, iv);

  // Codificar em Base64
  String encrypted = base64::encode(output, padded_len);
  return encrypted;
}
...
void loop() {
  ...
  if (now - lastMsg > intervalo) {
    ...
    // Enviar humidade do ar (sem criptografia por enquanto)
    dtostrf(humAr, 4, 1, buffer);
    client.publish(topic_humidade_ar, buffer);

    // Enviar humidade do solo (sem criptografia por enquanto)
